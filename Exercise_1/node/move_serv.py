...
def main():
    ...
    while(move=='Y'):
        ...
        move=rospy.wait_for_message("/event_in", String, timeout=None).data                          
        print ("WHILE",move)
        if (move=='Y'):                                                             #proceed if user says 'Y'
            
            dist=rospy.get_param("/distance", 0.0)   
            print ("IF MOVE",dist)
            if (dist!=0.0):                                                         #proceed if user enters value other than 0 
                direct=rospy.wait_for_message("/input", String, timeout=None).data
                print "Calling MOVE"
                print (direct,dist, move)
